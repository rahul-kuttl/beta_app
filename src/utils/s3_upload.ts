...
export const generatePresignedUrl = async (bucketName: string = 'alpha-file-storage', fileKey: string, expires = 60): Promise<string> => {
  try {
...
export const uploadFileToS3 = async (fileContent: Buffer, fileName: string, bucketName = 'alpha-file-storage'): Promise<any> => {
  try {
    await minioClient.putObject(bucketName, fileName, fileContent);
    const fileUrl = `${config.minioConfig.useSSL ? 'https' : 'http'}://${config.s3Config.s3_endpoint}/${bucketName}/${fileName}`;

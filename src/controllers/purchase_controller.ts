...
export const getPresignedUrlForUpload = async (req: Request, res: Response) => {
  try {
    ...
    res
      ...
      .send('An error occurred while generating the pre-signed URL.');
  }
};
...
export const viewMultiplePurchases = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(HTTP_INTERNAL_SERVER_ERROR)
      .send('An error occurred while fetching purchases.');
  }
};
...
export const addMultiplePurchases = async (req: Request, res: Response) => {
  try {
    ...
    if (!Array.isArray(req.body.purchases)) {
      return res
        .status(HTTP_BAD_REQUEST)
        .send('Invalid input format: purchases should be an array.');
    }
    ...
    res
      .status(HTTP_INTERNAL_SERVER_ERROR)
      .send('An error occurred while adding purchases.');
  }
};
 ...
 */
export const removeMultiplePurchaseItems = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (
      !Array.isArray(purchaseItemIds) ||
      purchaseItemIds.some((id) => !mongoose.isValidObjectId(id))
    ) {
      return res
        .status(400)
        .send('Invalid input: Expected an array of valid IDs');
    }
    ...
    res
      .status(200)
      .send(
        `Purchases deleted successfully. Total deleted: ${result.deletedCount}`,
      );
  } catch (error) {
 ...
 */
export const updateMultiplePurchaseItems = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (
      ...
      updates.some(
        (update) =>
          !update.id || !mongoose.isValidObjectId(update.id) || !update.data,
      )
    ) {
      return res
        .status(HTTP_BAD_REQUEST)
        .send('Invalid input: Each update must have a valid id and data');
    }

    const updatePromises = updates.map((update) =>
      Purchase.findByIdAndUpdate(update.id, update.data, { new: true }).catch(
        (err) => err,
      ),
    );
    ...
    res
      .status(HTTP_INTERNAL_SERVER_ERROR)
      .send('An error occurred while updating purchases.');
  }
};
...
export const viewPurchase = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(HTTP_INTERNAL_SERVER_ERROR)
      .send('An error occurred while fetching the purchase.');
  }
};
...
export const addPurchase = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(HTTP_INTERNAL_SERVER_ERROR)
      .send('An error occurred while adding the purchase.');
  }
};
...
export const removePurchaseItem = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(HTTP_INTERNAL_SERVER_ERROR)
      .send('An error occurred while deleting the purchase.');
  }
};
...
export const updatePurchaseItem = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(HTTP_INTERNAL_SERVER_ERROR)
      .send('An error occurred while updating the purchase.');
  }
};
